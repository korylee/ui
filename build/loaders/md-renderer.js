...
export default function createRenderer(wrapCodeWithCard = true) {
  ...
  const overrides = {
    ...
    code(code, language) {
      ...
      if (!isLanguageValid)
        throw new Error(`MdRenderer: ${language} is note valid for code -${code}`);
      const highlighted = hljs.highlight(code, { language }).value;
      const content = `<k-code><pre v-pre>${highlighted}</pre></n-code>`;
    ...
    codespan: (code) => `<k-text code>${code}</n-text>`,
